#!/usr/bin/env python3
"""Build Module 2 final tables v3 from existing Plan B result artifacts.
...
def write_main_latex(raw: dict[str, dict[str, Summary]]) -> None:
    ...
    lines = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Module 2 final learned-model comparison under final-tables-v3 protocol.}",
        r"\label{tab:module2_main_v3}",
...
def write_report(main_md: str, raw_main: dict[str, dict[str, Summary]], ablation_rows: list[dict[str, str]], parameter_rows: list[dict[str, str]], ours_all_best: bool) -> None:
    ...
    can_write = "可以"
    report = f"""# 模块2 final tables v3 封板报告
...
模块2是否还需要补实验：**{need_more_experiments}**。当前 hcf 同步结果已经足够进入 final tables v3 封板；继续补性能实验会破坏当前收口边界。
...
def main() -> None:
    ...
    print("Wrote final tables v3 artifacts:")
