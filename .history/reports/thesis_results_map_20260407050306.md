...
- **证明目的**：说明为什么LLM只提取“提及文本(Mention Text)”，而数值换算交由 Python 规则完成。证明工程设计的严谨性和对大语言模型“弱数学计算能力”的有效规避。

### 3. Phase 4 vs Phase 5 主实验对比柱状图 (Main Results Benchmark)
- **存放位置**：第五章 实验结果
- **来源建议**：根据 `phase5_main_results.md` 中的 F1 / AUPRC 表格生成柱状图。
- **证明目的**：直观展示大模型微调（Phase 5）在银标准上全面超越纯规则/Prompt 基线（Phase 4），尤其在 `density_category` 上的鸿沟差距。
...
### 表2：Phase 4 & Phase 5 主模型银标准评测核心指标汇总表
- **存放位置**：第五章 实验结果
- **内容来源**：整合 Phase 4 和 Phase 5 产出的 AUPRC 和 Macro-F1 结果。
- **证明目的**：用详实数字证明微调模型的有效性（证明方案走通了）。
